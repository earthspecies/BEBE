...
class sashimi(SupervisedBehaviorModel):
  ...
  def predict(self, data):
    ### We need a special predict method for Sashimi because of the pooling operations
    
    self.model.eval()
    
    alldata= data
    alldata_len = np.shape(alldata)[0]
    total_downsample = int(np.prod(self.pool)) 
...
#     print("there are %d S4 params and %d other params" % (len(params), len(all_parameters)-len(params)))
