...
#     tail = n_samples % self.n_samples_window
#     pad = (self.n_samples_window - tail) % self.n_samples_window
    
#     data = np.pad(data, ((0, pad), (0, 0)))
    
#     padded_data_len = np.shape(data)[0]
#     data = np.reshape(data, (-1, self.n_samples_window, n_features))
    
#     if labels is not None:
#       labels = np.pad(labels, (0, pad))
#       labels = np.reshape(labels, (-1, self.n_samples_window)) 

#       # extract labels using the mode
#       labels = np.squeeze(stats.mode(labels, axis = 1).mode)
    
#     # Compute summary statistics (features)
#     features = []
#     features.append(np.amax(data, axis = 1)) # max value
#     features.append(np.amax(data, axis = 1)) # min value
#     mu = np.mean(data, axis = 1)
#     features.append(mu) # mean
#     sigma = np.std(data, axis = 1)
#     features.append(np.std(data, axis = 1)) # std
#     features.append(stats.skew(data, axis = 1)) # skew
#     features.append(stats.kurtosis(data, axis = 1)) # kurtosis
#     features.append(vectorized_slope(data)) # slope (up to a constant)
#     features.append(autocorr(data, mu, sigma)) # approximate autocorrelation
...
#     return features, padded_data_len, labels
...
#       data = self.load_model_inputs(fp)
#       labels = self.load_labels(fp)
#       data, _ , labels = self.prepare_model_inputs(data, labels = labels)
#       train_data.append(data)
#       train_labels.append(labels)
...
#   def predict(self, data):
#     raw_data_size = np.shape(data)[0]
#     data, padded_data_size, _ = self.prepare_model_inputs(data)
#     predictions = self.model.predict(data)
    
#     # resize and crop predictions so they fit the size of the original data
#     predictions = resize(predictions, (padded_data_size,), order=0, mode='constant')
#     predictions = predictions[:raw_data_size]
#     return predictions, None
