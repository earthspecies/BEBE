...
class harnet(SupervisedBehaviorModel):
  def __init__(self, config):
    ...
    self.model = Classifier(self.n_features,
                            self.n_classes,
                            self.device,
                            self.harnet_version).to(self.device)
      
    print(self.model)

class Classifier(nn.Module):
    def __init__(self, n_features, n_classes, device, harnet_version):
        super(Classifier, self).__init__()
        
        harnet = torch.hub.load('OxWearables/ssl-wearables', harnet_version, class_num=5, pretrained=True).to(device).eval()
