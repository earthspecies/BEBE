...
def embedd_latent_vectors(cfg, files, model, legacy):
    ...
    #for file in ['train', 'test']:
        ...
        dataset = SEQUENCE_DATASET(os.path.join(cfg['project_path'],"data", "train",""), data=file+'_seq.npy', train=False, temporal_window=temp_win)
        ...
        dataloader = Data.DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=False, drop_last=False)
        
        with torch.no_grad():
            for idx, data_item in enumerate(dataloader):
                data_item = data_item.permute(0,2,1)
                data_item = data_item.type('torch.FloatTensor').cuda()
                h_n = model.encoder(data_item) 
                _, mu, _ = model.lmbda(h_n)
                latents = mu.cpu().data.numpy()
                latent_vector_list.append(latents)
                if idx % 1000 == 0:
                    print("I've looked at %d batches" % idx)
    ...
    if legacy == True:
        ...
        for folders in cfg['video_sets']:
            if not os.path.exists(os.path.join(cfg['project_path'],"results",folders,model_name,"")):
                os.makedirs(os.path.join(cfg['project_path'],"results",folders,model_name,""))
        ...
        # ^^ references variable "file" before it's been properly defined
        # For now, assume that if we are running this function, we want to redo the segmentation
        if True:
            new = True
            ...
            if ind_param == False:
                print("For all animals the same tk-Means parameterization of latent vectors is applied for %d cluster" %n_cluster)
        ...
        '''    
            ...
            if flag == 'yes':
                new = True
                path_to_latent_vector = os.path.join(cfg['project_path'],"results",file,model_name,'kmeans-'+str(n_cluster),"") # references variable "file" before it has been properly defined
                latent_vectors = []
                for file in files:
        ...
        if new == True:
            for idx, file in enumerate(files):
                ...
                if not os.path.exists(os.path.join(cfg['project_path'],"results",file,model_name,'kmeans-'+str(n_cluster),"")):                    
                    try:
                        os.makedirs(os.path.join(cfg['project_path'],"results",file,"",model_name,'kmeans-'+str(n_cluster),""))
                ...
                np.save(os.path.join(save_data,'km_label_'+file), labels[idx])
